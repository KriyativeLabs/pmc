...
object Users {
  ...
  def findById(id: Int,companyId:Option[Int]=None): Option[User] = {
    val filterQuery = if(companyId.isDefined) userQuery.filter(x => x.id === id && x.companyId === companyId.get) else userQuery.filter(x => x.id === id )
