...
case class Connection(id:Option[Int], customerId:Int, setupBoxId:String,planId:Int,discount:Int,installationDate:DateTime,
                      status:String, cafId:String,idProof:String,companyId:Int)
object Connection {
...
class ConnectionsTable(tag: Tag) extends Table[Connection](tag, "connections"){
  ...
  def * = (id.?,customerId, setupBoxId,planId,discount,installationDate, status, cafId,idProof,companyId) <> ((Connection.apply _).tupled, Connection.unapply _)
}
