...
object DashboardController extends Controller with DashboardSerializer with CommonUtil with ResponseHelper {
  ...
  def dashboardData = (IsAuthenticated andThen PermissionCheckAction(UserType.OWNER)) { implicit request =>
  ...
  def monthlyStatistics = (IsAuthenticated andThen PermissionCheckAction(UserType.OWNER)) { implicit request =>
    val result = CompanyStats.getLatestByCount(request.user.companyId)
    ok(Json.toJson(result),"Monthly Statistics")
  }

  def agentStatistics = (IsAuthenticated andThen PermissionCheckAction(UserType.OWNER)) { implicit request =>
